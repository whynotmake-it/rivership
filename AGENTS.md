...
- `melos analyze` uses `dart analyze --fatal-infos`, so info-level lints fail the run. There is no committed lockfile, so upstream deprecations (e.g. `equatable`) can surface as new infos.
- `apps/example`, `heroine/example`, and `stupid_simple_sheet/example` use the `liquid_glass_renderer` git package, pinned to the commit of [flutter_liquid_glass#154](https://github.com/whynotmake-it/flutter_liquid_glass/pull/154), which makes its shaders SkSL-compatible for web on Flutter 3.44. With that pin, the GitHub Pages build (`.github/workflows/deploy.yaml`: `cd apps/example && flutter build web --release --wasm`) succeeds. Older pins fail web builds with `ShaderCompilerException`. Do not enable flutter_gpu.
- `apps/example` ships only `macos/` + `web/` platform folders and the per-package examples ship none. Run `apps/example` on web, or scaffold a platform temporarily (`flutter create --platforms web .`) for a per-package example and don't commit it.

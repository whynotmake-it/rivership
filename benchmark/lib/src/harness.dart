...
class ScenarioResult {
  ...
  Map<String, Object?> toJson() => {
        ...
        'layers': {
          for (final layer in {for (final s in [...motor, ...flutter]) s.layer})
            layer.name: {
...
void printResults(List<ScenarioResult> results, BenchConfig config) {
  ...
  for (final layer in layers) {
    ...
    for (final r in results) {
      ...
      final p90Delta = base.p90 == 0
          ? 0.0
          : (prim.p90 - base.p90) / base.p90 * 100;
...
File writeResultsJson(
  ...
  final relative = path ??
      'results/bench_${DateTime.now().millisecondsSinceEpoch}.json';

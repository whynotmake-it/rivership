...
Each entry says what changed, how 1.x behaved, and how to migrate.
 ...
 - **BREAKING**: a spring now finishes when it is done for good, within its `tolerance`, in `MotionController`, the sequence controllers and builders, and tracks. In 1.x an animation or sequence phase ended on the first frame where `isDone` was true, which for an underdamped spring can be near a peak, before it swings out again. In those cases futures complete, `status` changes and the next phase starts up to about 0.3 s later with the presets (more for bouncier springs), while the value moves by less than the tolerance. The spring's motion is unchanged.
...
### Motion type hierarchy
 ...
 - **FEAT**: add `Motion.settlingDuration(start:, end:, velocity:)`, which takes the same arguments as `createSimulation` and returns when that simulation is done, or `null` when unknown. Curves and `NoMotion` return their duration, springs compute exactly when they settle within their `tolerance` (with no sampling), and wrappers ask their parent. Tracks, `.at` steps, loops and inspection tools end every step at this time; for `null`, motor finds the end by sampling the simulation as before. A spring step now ends when the spring is done for good: an underdamped spring whose `isDone` briefly reported done near a peak used to end there, up to about 0.3 s early for the presets, with the rest of the motion within the tolerance. `CupertinoMotion.duration` stays the spring's perceptual pace, not when it finishes.

...
### Motion type hierarchy
 ...
 - **FEAT**: add `Motion.cutAfter(Duration)`, which returns a `CutMotion`: the motion plays at its own speed and ends at exactly that duration, landing on its target there. Where it hasn't arrived yet, the played part is corrected in proportion to its progress, so the start value and velocity are kept (for a spring from rest, the move is scaled by about 1.4%). The velocity at the cut is handed to the next step, and a last step stops there. `CupertinoMotion.cutShort()` cuts the spring at its perceptual `duration`, which keeps its 1.x meaning.

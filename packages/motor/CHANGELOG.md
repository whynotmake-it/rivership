...
Each entry says what changed, how 1.x behaved, and how to migrate.
 ...
 - **BREAKING**: a `MotionConverter` must not keep or change the lists motor hands it. `denormalize` now receives motor's reused buffer, which changes after the call, and motor may keep the list `normalize` returns. In 1.x, `denormalize` got a fresh list each time, so keeping it was safe. Only motor's own converters got the buffer directly; custom converters paid for a copy on every read. Converters that build a new value from the numbers, as nearly all do, are unaffected and now read as fast as the built-in ones. If yours returns or stores the list itself (for example `denormalize: (values) => values`), copy it: `List.of(values)`.
 - **BREAKING**: motor no longer depends on `equatable`. `MotionSequence` and its subclasses (`StepSequence`, `StateSequence`, `SpanningSequence`, `SingleMotionPhaseSequence`) no longer mix in `EquatableMixin`, so they have no `props` or `stringify`, and `is EquatableMixin` is false. They still compare by value (same class, phases, values, motions that move the same, and loop mode), and a custom subclass gets that comparison without overriding anything. In 1.x a subclass could customize equality by overriding `props`; override `==` and `hashCode` instead. `toString` no longer depends on `EquatableConfig.stringify`. If you relied on `props`, compare the sequences directly.
 - **BREAKING**: classes that `implements Motion` must now also provide `settlingDuration(...)` and `scaleTo(Duration)`, which 2.0 added to `Motion` and `MotionBase` with default implementations. Classes that `extends Motion` inherit those defaults and keep compiling; extending is the recommended way to write a custom motion. See [MIGRATION.md](./MIGRATION.md).
...
### Tracks: a new multi-property animation system
 ...
 - **FEAT**: add `TrackStep<T>`, the unit of a track animation: `TrackStep.to` (animate to a target), `TrackStep.at` (a keyframe that arrives exactly at an absolute time), `TrackStep.hold`, `TrackStep.free` (run a self-directed `FreeMotion`), and `TrackStep.sync` (a barrier shared by tracks through a token). `.at` stretches its motion over the gap when the preceding step ends early enough, and otherwise cuts the preceding step so its motion runs its natural length (its `settlingDuration`); the two cases meet continuously, and with no time left the value arrives instantly.
 - **FEAT**: add `TrackAnimation<T>` (one track's steps plus its own `from`/`withVelocity`), `TrackTimeline` (a reusable multi-track clip that compares by value and owns its `LoopMode`), and `TrackPhaseTimeline<P>` (named phases with sync barriers at phase boundaries, one-time `initialValues`/`initialVelocities`, `phaseLoop`, and the phases as a plain clip via `flattened`). `TrackController(initialValues:)` sets a track's value the first time the controller sees it.
...
### Motion type hierarchy
 ...
 - **FEAT**: add `Motion.settlingDuration(start:, end:, velocity:)`, which takes the same arguments as `createSimulation` and returns when that simulation is done, or `null` when unknown. Curves and `NoMotion` return their duration, springs compute exactly when they settle within their `tolerance` (with no sampling), and wrappers ask their parent. Tracks, `.at` steps, loops and inspection tools end every step at this time, and ask for it only when they need it; for `null`, motor finds the end by sampling the simulation as before. `CupertinoMotion.duration` stays the spring's perceptual pace, not when it finishes.
 - **FEAT**: add `Motion.cutAfter(Duration)`, which returns a `CutMotion`: the motion plays at its own speed and ends after exactly that duration, landing on its target there. Where it hasn't arrived yet, the played part is corrected in proportion to its progress, so the start value and velocity are kept (for a spring from rest, the move is scaled by about 1.4%). The velocity at the cut goes to the next step, and a last step stops there. `skipTail()` cuts a spring at its perceptual duration, for example `Motion.bouncySpring().skipTail()` ends after 500 ms instead of settling until about 1.7 s. `CupertinoMotion.duration` keeps its 1.x meaning: the spring still settles after it unless you call `skipTail()`.
...
### Fixes
 ...
 - **FIX**: steps, track animations, timelines and the deprecated sequences compare the motions they hold with the motions' own `==`, which is equal when two motions produce the same movement, whatever their class. Before, a sequence using `Motion.linear(d)` didn't equal one using `Motion.curved(d)`, although the motions were equal, so a rebuild could restart it.
 - **FIX**: built-in converters compare equal by type, and `MotionConverter.custom`/`customDirectional` converters by their functions. Before, a converter created without `const` never equaled the previous one, so a `MotionBuilder` rebuilt with it swapped converters on every build, which in 2.0 also reset velocity tracking.
 ...
 - **FIX**: a spring finishes when it is done for good, within its `tolerance`, in `MotionController`, the sequence controllers and builders, and tracks. In 1.x an animation or sequence phase ended on the first frame where `isDone` was true, which for an underdamped spring can be near a peak, just before it swings out again. In those cases futures now complete, `status` changes and the next phase starts up to about 0.3 s later with the presets (more for bouncier springs), while the value moves by less than the tolerance in that time. The spring's motion is unchanged.

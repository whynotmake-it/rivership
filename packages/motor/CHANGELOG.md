...
### Motion type hierarchy
 ...
 - **FEAT**: export `CurveSimulation`, the simulation behind `CurvedMotion`, as a `final class`. A custom motion that returns it from `createSimulation` plays as fast as `CurvedMotion`: dimensions that share its curve and duration evaluate the curve once per frame, and its end is known without searching.

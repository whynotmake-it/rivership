...
### Migrating to 2.0.0 (Latest)

**⚠️ BREAKING CHANGES:** Version 2.0.0 removes all legacy Spring widgets. All legacy classes have been completely removed and are no longer available.

#### Removed Classes (No Migration Path)
The following classes have been **completely removed** and are no longer available:
- `SpringBuilder` → Use `SingleMotionBuilder`
- `SpringBuilder2D` → Use `MotionBuilder` 
- `SpringDraggable` → Use `MotionDraggable`
- `SpringSimulationController` → Use `SingleMotionController`
- `SpringSimulationController2D` → Use `MotionController`
- `SimpleSpring` → Use `CupertinoMotion` or `Motion.spring(SpringDescription.withDurationAndBounce(...))`
...
// New
CupertinoMotion.standard   // Default iOS spring
CupertinoMotion.bouncy
CupertinoMotion.smooth  
CupertinoMotion.snappy
CupertinoMotion.interactive

// Or create custom springs
Motion.spring(SpringDescription.withDurationAndBounce(
  duration: Duration(milliseconds: 500),
  bounce: 0.2,
))
```
...
```dart
// SpringBuilder → SingleMotionBuilder
// Old (removed)
SpringBuilder(
  value: 0.5,
...
```dart
// SpringDraggable → MotionDraggable  
// Old (removed)
SpringDraggable(
  data: icon,
  spring: Spring.bouncy,
...
```dart
// SpringSimulationController → SingleMotionController
// Old (removed)
...
If you were using the deprecated classes in 1.0.0, they have now been completely removed. Follow the migration examples above.

### Migrating to 1.0.0 (Historical)

> **⚠️ Important:** Version 1.0.0 introduced major API changes including renaming `Spring` to `CupertinoMotion` and replacing `SpringMotion` with `Spring` wrapper around Flutter SDK's `SpringDescription`. If you used negative bounce values with `SimpleSpring`, the Flutter SDK's `SpringDescription.withDurationAndBounce` uses different mathematical formulas which may result in different spring behaviors.

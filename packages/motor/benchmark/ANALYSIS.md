...
- **Baseline:** the motor 2.0 engine before the optimization pass
  (three invocations of 7 runs each).
- **Final:** the engine as released, after it (five invocations). Earlier
  final runs during development gave the same animation ratios within noise.

The optimization pass changed four things:
- Segment ends are found as time passes, not searched up front.
- Velocities are computed only when read, and dimensions that share a curve
  evaluate it once per frame.
- Fewer allocations on start and per frame, and less memory retained per
  track.
- Velocity samples go into a preallocated buffer, and `set` does less
  bookkeeping.

The same work also fixed `CurveSimulation.dx`, which reported 4× the real
velocity.

The individual commits are in the development pull request,
[#296](https://github.com/whynotmake-it/rivership/pull/296).
...
Before the preallocated buffer, each tracked sample cost about 0.6 µs and 370 B. About
...
This file used to track per-phase numbers from the original harness
(definev, #307). That harness timed `WidgetTester.pump` under
`flutter test`, used a looping curve for multi-track, and read uncurved
`AnimationController.value`. Its tables are in the development pull request,
[#296](https://github.com/whynotmake-it/rivership/pull/296). The widget
rebuild, manual `set` and velocity tracking scenarios were dropped with it;
the drag scenarios replace the last two.
They measured widget or `set` paths rather than the animation engine; their
numbers are in the same pull request.

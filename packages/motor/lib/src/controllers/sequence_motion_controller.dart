...
class SequenceMotionController<P, T extends Object>
  ...
  /// Provides the future [playSequence] returns for a looping sequence.
  ///
  /// Each cycle is its own run, so the runs' futures complete between cycles.
  /// This ticker is muted, so it never schedules frames; it is only stopped
  /// when the sequence ends, like the ticker of `AnimationController.repeat`.
  Ticker? _loopTicker;
  ...
  /// For looping sequences it never completes on its own: [stop] completes
  /// it, and a canceled stop or any other interruption cancels it.
  ///
  ...
  TickerFuture playSequence(
    ...
    final future = _playChain(
      sequence.phases.sublist(startIndex),
      fromPhaseForFirstLeg: null,
      withVelocity: withVelocity,
      emitTransition: false,
    );
    if (!sequence.loop.isLooping) return future;
    return (_loopTicker = Ticker((_) {})..muted = true).start();
  }

  void _endLoop({required bool canceled}) {
    final ticker = _loopTicker;
